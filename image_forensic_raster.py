...
_FIGURE_RE = re.compile(
    r'(?:Supplementary\s+)?'        # préfixe optionnel "Supplementary"
    r'(?:FIGURE|Figure|Fig\.?)'     # mot-clé
    r'\s*'                          # espace(s) éventuel(s)
    r'([Ss]?[0-9]+[A-Za-z]?)',       # numéro : S1, S10, 1, 2A, S2B…
    re.IGNORECASE
)
...
# ══════════════════════════════════════════════════════════════════════════════
# Rendu des pages vectorielles (figures non-raster)
# ══════════════════════════════════════════════════════════════════════════════

def find_caption_y(pdf_path, page_idx):
    """
    Localise la légende "Figure SX." via pdfplumber (coordonnées texte).
    Retourne (y_points, page_height_points) ou (None, None).
    """
    ...
    try:
        with _pdfplumber.open(pdf_path) as pdf:
            ...
            for i, w in enumerate(words):
                if re.match(r'Figure', w['text'], re.I):
                    if i + 1 < len(words) and re.match(r'[Ss]?\d+', words[i+1]['text']):
                        return w['top'], page.height
    except Exception:
        pass
    return None, None


def render_vector_pages(pdf_path, figure_map, dpi=150):
    """
    Rastérise les pages vectorielles via pypdfium2 (pas de dépendance système).
    Ne traite que les pages listées dans figure_map sans image raster.
    Retourne une liste de dicts compatibles avec analyze_image().
    """
    if not _RENDER_AVAILABLE:
        return []

    scale = dpi / 72.0  # points PDF (72 dpi) → pixels cible

    try:
        doc = _pypdfium2.PdfDocument(pdf_path)
    except Exception as e:
        print(f"  ⚠  Ouverture PDF impossible : {e}", file=sys.stderr)
        return []

    rendered_images = []
    for page_num, labels in sorted(figure_map.items()):
        idx = page_num - 1
        if idx >= len(doc):
            continue
        try:
            # Rastérisation via pypdfium2 — scale=dpi/72
            page = doc[idx]
            bitmap = page.render(scale=dpi / 72.0)
            rendered = bitmap.to_pil().convert("RGB")

            # Rogner au-dessus de la légende pour isoler la figure
            y_cap, _ = find_caption_y(pdf_path, idx)
            if y_cap and y_cap > 50:
                y_px = int(y_cap * scale) - 10
                cropped = rendered.crop((0, 0, rendered.width, max(100, y_px)))
            else:
                cropped = rendered

            fig_label = ", ".join(dict.fromkeys(labels))
...
def extract_images_from_pdf(pdf_path, min_size=50, max_aspect_ratio=5.0):
    """
    Extrait les images scientifiques du PDF en excluant automatiquement :
      - Les doublons  (même hash MD5 → logo répété sur chaque page)
      - Les bandeaux / logos  (ratio largeur/hauteur > max_aspect_ratio)
      - Les images trop petites  (< min_size px sur un côté)

    max_aspect_ratio=5.0 : un ratio > 5 est typique d'un bandeau horizontal
    (ex : logo journal 688×85 → ratio 8.1). Les vraies figures scientifiques
    ont un ratio généralement compris entre 0.3 et 3.
    """
    ...
    for page_num in range(len(doc)):
        ...
        for img_idx, img_info in enumerate(page.get_images(full=True)):
            ...
            try:
                ...
                # Filtre 1 : doublons (logo répété sur chaque page)
                ...
                # Filtre 2 : trop petite
                if w < min_size or h < min_size:
                    continue

                # Filtre 3 : bandeau / logo (ratio extrême)
                aspect = w / h
...
def analyze_image(img_data, ela_quality=95):
    ...
    #    Désactivé pour les pages vectorielles rendues (trop de faux positifs :
    #    axes, marges et textes répétés créent des features similaires artificiellement)
    is_rendered = img_data.get("is_rendered", False)
    if pil.width > 400 and pil.height > 200 and not is_rendered:
        try:
        ...
        report.findings.append(Finding("Inter-panneaux", "info",
            "Non applicable aux figures vectorielles rendues (faux positifs)", 0.0))
...
def generate_html_report(pdf_path, reports, output_path):
    ...
    for r in reports:
        ...
        # Badge figure
        # Badge couleur : bleu = raster, violet = vectoriel rendu, gris = inconnu
        is_rendered = any(f.technique == "Inter-panneaux" and "vectorielle" in f.description
                          for f in r.findings)
        if r.figure_label:
            badge_color = "#6a3d9a" if is_rendered else "#2c3e50"
            badge_suffix = " ⃝v" if is_rendered else ""
            fig_badge = (
                ...
                f'margin-bottom:5px" title="{"Page vectorielle rendue" if is_rendered else "Image raster"}">'
                f'{r.figure_label}{badge_suffix}</span><br>'
            )
        else:
            fig_badge = (
...
<body>
  ...
  <div class="legend">
    ...
    • <b>Inter-panneaux</b> : similarité anormale entre colonnes d'une figure composite —
      deux conditions expérimentales ne devraient pas partager autant de features visuels.
      <em>Non appliqué aux figures vectorielles (trop de faux positifs)</em><br>
    • <b>Badge violet ⃝v</b> : figure vectorielle (graphique, courbe) rastérisée pour analyse —
      seuls ELA et bruit sont fiables sur ce type d'image<br>
...
def main():
    ...
    parser.add_argument("--max-ratio", type=float, default=5.0,
                        help="Ratio largeur/hauteur max pour filtrer logos/bandeaux (défaut : 5.0)")
    ...
    # 2b. Rendu des pages vectorielles (figures non raster)
    raster_pages = set(img["page"] for img in images)
